package com.rootlens.payment_service;
...
public class LogForwarder {
    ...
    public void sendMetric(String service, String metricName, double value) {
        if (ingestionUrl.isBlank()) return;
        long ts = System.currentTimeMillis() / 1000;
        String json = "{\"service\":\"" + esc(service) + "\""
                + ",\"metricName\":\"" + esc(metricName) + "\""
                + ",\"value\":" + value
                + ",\"timestamp\":" + ts + "}";
        try {
            HttpHeaders h = new HttpHeaders();
            h.setContentType(MediaType.APPLICATION_JSON);
            restTemplate.postForObject(ingestionUrl + "/metrics", new HttpEntity<>(json, h), String.class);
        } catch (Exception e) {
            log.warn("Metric ingestion unreachable: {}", e.getMessage());
        }
    }
